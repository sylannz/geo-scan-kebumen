...
async function handleAskGemini(e) {
  ...
  try {
    const res = await fetch(
      `${CONFIG.geminiChatEndpoint}?key=${CONFIG.geminiApiKey}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        ...
        body: JSON.stringify({
          ...
          systemInstruction: {
            parts: [{
              text: `Geo, Ahli Geologi Kebumen. Singkat (3-4 kalimat). Bahasa: ${STATE.language === "id" ? "Indonesia" : "English"}.`
            }]
          }
        })
      }
    ...
    messages.innerHTML += `<div class="flex justify-start"><div class="p-4 rounded-2xl max-w-[85%] text-xs bg-red-50 text-red-700 rounded-bl-none border border-red-200">Error: ${err.message}</div></div>`;
  } finally {
