...
const TRANSLATIONS = {
  id: {
    ...
    eduDesc: 'Konservasi Penyu Kaliratu adalah pusat pelestarian penyu lekang dan penyu hijau yang dikelola secara swadaya oleh masyarakat desa Jogosimo. Biosite ini menjadi garda terdepan dalam melindungi telur penyu dari predator dan perburuan liar.',
    unescoTitle: 'Mengapa Diangkat UNESCO?',
    unescoDesc: 'Sebagai biosite resmi, Kaliratu membuktikan bahwa pilar konservasi UNESCO mencakup hubungan harmonis antara manusia dan ekosistem hayati. Keberhasilan penetasan ribuan tukik setiap tahunnya menjadi indikator kesehatan lingkungan pesisir internasional.',
    legendTitle: 'Mengapa disebut "Kaliratu"?',
    legendDesc: 'Kaliratu berasal dari pertemuan muara sungai dan laut kidul yang dianggap sakral. Area ini menjadi jalur alami bagi sang "Ratu Samudra" yaitu penyu untuk kembali pulang dan menitipkan generasi penerusnya di pasir Jogosimo.',
    geoAnalysisTitle: 'Analisis Biosite',
    chertTitle: 'Penyu Lekang (Olive Ridley)',
    chertDesc: 'Spesies penyu yang paling setia mendarat di pesisir selatan. Perannya sangat vital dalam menjaga keseimbangan rantai makanan laut.',
    lavaTitle: 'Habitat Pantai Pasir',
    lavaDesc: 'Karakteristik pasir yang hangat dan bersih di Jogosimo mendukung inkubasi telur penyu secara alami dengan tingkat keberhasilan tinggi.',
    audioTitle: 'Pemandu Suara AI',
    audioDesc: 'Halo semuanya! Selamat datang di Biosite Konservasi Penyu Kaliratu, Kebumen Geopark. ',
    ...
    time1Desc: 'Warga Jogosimo mulai melindungi telur penyu dari ancaman predator dan perburuan.',
    ...
    chatWelcome: 'Halo! Saya Asisten Geo. Ada yang ingin kamu ketahui tentang Penyu Kaliratu?',
  ...
  en: {
    ...
    eduDesc: 'Kaliratu Turtle Conservation is a preservation center for Olive Ridley and Green turtles managed independently by Jogosimo village community. This biosite serves as the frontline in protecting turtle eggs from predators and illegal hunting.',
    unescoTitle: 'Why UNESCO Recognition?',
    unescoDesc: 'As an official biosite, Kaliratu proves that UNESCO conservation pillars include harmonious relationships between humans and biological ecosystems. The successful hatching of thousands of hatchlings annually serves as an indicator of international coastal environmental health.',
    legendTitle: 'Why is it called "Kaliratu"?',
    legendDesc: 'Kaliratu comes from the meeting of river mouth and south sea considered sacred. This area becomes a natural path for the "Ocean Queen" (turtles) to return home and entrust their next generation in Jogosimo\'s sand.',
    geoAnalysisTitle: 'Biosite Analysis',
    chertTitle: 'Olive Ridley Turtle',
    chertDesc: 'The most loyal turtle species landing on the southern coast. Its role is vital in maintaining the marine food chain balance.',
    lavaTitle: 'Sandy Beach Habitat',
    lavaDesc: 'The warm and clean sand characteristics in Jogosimo support natural turtle egg incubation with high success rates.',
    audioTitle: 'AI Voice Guide',
    audioDesc: 'Hello everyone! Welcome to Kaliratu Turtle Conservation Biosite, Kebumen Geopark. ',
    ...
    time1Desc: 'Jogosimo residents began protecting turtle eggs from predator and poaching threats.',
    time4Date: 'September 9, 2024',
    time4Event: 'UNESCO GLOBAL GEOPARK',
    time4Desc: 'Kaliratu is officially recognized as a world-class UNESCO biosite.',
    aiChatTitle: 'Ask Geo ✨',
    chatWelcome: 'Hello! I am Geo Assistant. What would you like to know about Kaliratu Turtle?',
    chatFabLabel: '✨ Chat Geo',
...
function updateUI() {
  ...
  document.getElementById('edu-title').innerHTML = `<i data-lucide="book-open" class="w-5 h-5 text-sky-600"></i> ${t.eduTitle}`;
  document.getElementById('edu-desc').innerText = t.eduDesc;
  document.getElementById('unesco-title').innerHTML = `<i data-lucide="check-circle" class="w-4 h-4 text-blue-600"></i> ${t.unescoTitle}`;
  document.getElementById('unesco-desc').innerText = t.unescoDesc;
  document.getElementById('legend-title').innerHTML = `<i data-lucide="landmark" class="w-4 h-4 text-amber-500"></i> ${t.legendTitle}`;
  document.getElementById('legend-desc').innerText = t.legendDesc;
  document.getElementById('geo-analysis-title').innerHTML = `<i data-lucide="microscope" class="w-4 h-4 text-sky-600"></i> ${t.geoAnalysisTitle}`;
...
async function handleAudioPlay() {
  ...
  document.getElementById('audio-progress-container').classList.remove('hidden');
  ...
  try {
    ...
    const utterance = new SpeechSynthesisUtterance(fullText.replace(t.audioDesc, ''));
...
function updateAudioBtn(isPlaying) {
  ...
  document.getElementById('audio-progress-container').classList.toggle('hidden', !isPlaying);
  lucide.createIcons();
}

function switchTab(tab) {
  const isOverview = tab === 'overview';
  document.getElementById('view-overview').classList.toggle('hidden', !isOverview);
  document.getElementById('view-history').classList.toggle('hidden', isOverview);
  
  document.getElementById('tab-overview').className = `pb-2 text-xs font-bold uppercase tracking-wider transition-all ${isOverview ? 'text-sky-600 border-b-2 border-sky-600' : 'text-slate-400'}`;
  document.getElementById('tab-history').className = `pb-2 text-xs font-bold uppercase tracking-wider transition-all ${!isOverview ? 'text-sky-600 border-b-2 border-sky-600' : 'text-slate-400'}`;
}
...
function initCarousel() {
  ...
  setInterval(() => {
    ...
    document.getElementById(`dot-${prev}`).className = 'h-1 rounded-full transition-all duration-300 w-2 bg-white/50';
    document.getElementById(`dot-${STATE.currentSlide}`).className = 'h-1 rounded-full transition-all duration-300 w-6 bg-sky-500';
