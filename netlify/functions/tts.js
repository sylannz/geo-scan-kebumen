...
export const handler = async (event) => {
  try {
    ...
    if (!text || typeof text !== 'string') {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: 'Invalid request', message: 'Text is required and must be a string' })
      };
    }
    ...
    if (!language || !['id', 'en'].includes(language)) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: 'Invalid request', message: 'Language must be "id" or "en"' })
      };
    }

    const sanitizedText = text.substring(0, 1000);
    console.log(`[TTS] Netlify function called. language=${language} length=${sanitizedText.length}`);
    ...
    if (!response.ok || isError) {
      ...
      return {
        statusCode: 400,
        body: JSON.stringify({ error: 'TTS service error', message: responseText || 'Invalid audio response from VoiceRSS' })
      };
    }
